...
- 对“修改项目基础信息后，新的项目时间区间必须覆盖全部阶段节点时间区间”这类父级范围更新，优先把 `project` 行作为并发串行化根；只要创建节点、修改节点日期、删除节点等写接口也都先锁同一条 `project` 行，项目侧就不要再额外锁全量 `project_node` 或继续扩锁到 `sub_task`。
- 这类父级范围校验不要在 `Service` 里先查节点 `MIN/MAX` 日期再判断，优先在最后那条原子 `UPDATE ... FROM ... RETURNING` 里一次性算出更新后的父级日期并校验是否仍能覆盖全部直接子级；写返回空后再统一映射成时间区间冲突。
