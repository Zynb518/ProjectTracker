...
| --- | --- | --- |
| `status` | 否 | 按状态筛选 |
| `project_keyword` | 否 | 按项目名称做大小写不敏感的模糊包含匹配筛选 |
...
- `project_keyword` 使用 `ILIKE '%keyword%'` 语义，输入 `平台` 可匹配 `内部进度平台`
- 本接口不再接收 `project_id`
- 若当前用户负责的多个项目名称相同，则会按名称一并命中并返回对应任务
...
- 仅对当前状态为 `已完成` 的子任务开放
- 撤销完成后，优先恢复最近一条“未完成状态”历史中的 `status` 与 `progress_percent`
- 若不存在任何“未完成状态”历史，则恢复为 `status=1`、`progress_percent=0`
- 恢复后若按北京时间判断当前日期已超过 `planned_end_date`，则最终状态应落为 `4=已延期`
- 本接口只负责撤销完成，不负责后续编辑
...
- 健康检查接口服务于部署探活、运维排查和后续自动化检查
- 路径统一放在 `/api` 下，复用当前 `nginx -> /api -> Drogon` 的正式代理链路
- 第一版只做“应用存活”和“数据库就绪”两类最小检查，不扩展到本地 AI、前端静态资源或其他外部依赖
- 健康检查接口不依赖登录态，不挂登录过滤器
